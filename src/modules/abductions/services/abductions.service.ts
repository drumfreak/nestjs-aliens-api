...
import { AbductionsListRequestDTO, AbductionCreateRequestDTO } from '../dto';

@Injectable()
export class AbductionsService {
  private readonly logger = new Logger(AbductionsService.name);
  private repository: Repository<Abduction>;
  ...
  constructor(
    ...
    this.repository = this.dataSource.manager.getRepository(Abduction);
  ...
  async getById(id: any) {
    try {
      ...
      if (f) {
        ...
        throw new NotFoundException('Unable to find Abduction');
      }
    } catch (error: any) {
  ...
  async save(item: AbductionCreateRequestDTO) {
    try {
  ...
  async findOrCreate(c2: any) {
    try {
      ...
      if (f) {
        return {
          status: 'fail',
          message: 'Abduction already exists. Try updating instead.',
          result: f,
        };
      } else {
        ...
        if (f2) {
          ...
          throw new InternalServerErrorException('Unable to create abduction');
        }
      }
    } catch (error) {
      this.logger.warn(error);
  ...
  async update(id: any, body: any) {
    try {
      const item: Abduction = await this.getById(id);
      if (item?.id) {
  ...
  async search(body: any) {
    try {
      console.log('Abductions Search Body', body);
      return {
        status: 'success',
  ...
  async list(body: AbductionsListRequestDTO) {
    try {
